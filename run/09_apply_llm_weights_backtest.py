...
# Apply LLM-inferred alpha weights (from infer_lora_output.json) and run portfolio backtest.
...
def calmar(nav: pd.Series, rets: pd.Series, ann_factor=52) -> float:
    mdd = max_drawdown(nav)
    if not np.isfinite(mdd) or mdd >= 0:
        return float("nan")
    ann_ret = (1.0 + rets.dropna()).prod() ** (ann_factor / max(len(rets.dropna()), 1)) - 1.0
...
def build_weekly_weights_from_score(
    ...
    # keep last score in the week for each ticker
    ...
    # ---- make weights per reb_date ----
    def _make_weights_one_reb(g: pd.DataFrame) -> pd.DataFrame:
        # reb_date from group key is safest
...
def smooth_weights_ema(weights: pd.DataFrame, eta: float = 0.3) -> pd.DataFrame:
    """
    eta in [0,1].  eta越大越“粘住上一期”(更稳、换手更低)
    w_t = eta * w_{t-1} + (1-eta) * w_target
    """
    if weights is None or weights.empty:
        return pd.DataFrame()
    eta = float(eta)
    eta = min(0.999, max(0.0, eta))
    ...
    for dt, row in W.iterrows():
        ...
        if prev is None:
            ...
            y = eta * prev + (1.0 - eta) * x
        # 保持同样的 gross exposure（长短策略：sum abs 不一定=1，你按你原逻辑即可）
        prev = y
        out.append(pd.Series(y, index=cols, name=dt))
...
def load_infer_json(path: str) -> Tuple[List[str], Dict[str, float]]:
    ...
    # keep only selected keys, non-negative, normalize
    w = {}
    for a in selected:
        v = weights.get(a, None)
        try:
            v = float(v)
        except Exception:
            v = 0.0
...
def load_direction_from_metrics(metrics_csv: Optional[str], alphas: List[str]) -> Dict[str, float]:
    ...
    # 尽量兼容你 csv 的列名：alpha + sharpe_net 或 ic_mean
    if "alpha" not in m.columns:
        return out

    m["alpha"] = m["alpha"].astype(str)

    # 方向：优先看 sharpe_net 的正负，否则 ic_mean 的正负
    sign_col = None
...
def main():
    ...
    ap.add_argument("--zscore", action="store_true", help="Cross-sectional zscore by date before combining")
    ap.add_argument("--clip_z", type=float, default=None, help="Clip zscore/signal at +/- clip_z")
    ...
    if os.path.isdir(args.llm_dir):
        # parquet dataset dir or a parquet file (pandas can read dataset dir if valid)
        try:
            # 如果目录里没有 parquet，这里会异常或读不到；就跳过
            found = False
    ...
    # Build a per-alpha long table, then weighted sum per (date,ticker)
    parts = []
    for a in selected:
        ...
        # apply sign(direction) and alpha-weight
    ...
    summary = {
        ...
        "var_5pct_net": v5,
        "cvar_5pct_net": c5,
        "skew_net": float(net.dropna().skew()) if len(net.dropna()) > 10 else float("nan"),
        "kurt_net": float(net.dropna().kurtosis()) if len(net.dropna()) > 10 else float("nan"),
    }
